        ...
        */
        public DataTable consultarCasoEjecutado(int idEjecucion)
        {
            ...
            try
            {
                ...
                consulta = "SELECT C.id_caso, C.id_tipoNC, C.justificacion, C.imagen, C.estado_ejecucion, C.id_tipoNC" + " FROM Caso_Ejecutado C join Tipo_NC T on C.id_tipoNC = T.id_tipoNC where id_ejecucion  = '" + idEjecucion + "';";
                dt = acceso.ejecutarConsultaTabla(consulta);
            }
            catch (SqlException e)
            {
